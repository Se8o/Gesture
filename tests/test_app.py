...
class TestDrawHand:
    ...
    def test_does_not_raise(self, frame, landmarks):
        from src.app import _draw_hand
...
class TestRunStartup:
    ...
    def test_exits_if_camera_fails(self, tmp_path, monkeypatch):
        ...
        with (
            patch("src.app.GestureRecognizer", return_value=mock_recognizer),
            patch("src.app.cv2.VideoCapture",  return_value=mock_cap),
...
class TestHandConnections:
    def test_all_indices_in_range(self):
        from src.app import HAND_CONNECTIONS
